...
resource "aws_eks_node_group" "istio-node-grp" {
  ...
  # ✅ KEEP TAINT: Only Istio/Keycloak pods should run here
  taint {
    key    = "node-role"
    value  = "osdu-istio-keycloak"
    effect = "NO_SCHEDULE"
...
#########################################################################
# NODE GROUP 2: BACKEND DATABASE NODES (TAINTED)
# Purpose: PostgreSQL, Elasticsearch, MinIO, RabbitMQ ONLY
#########################################################################
...
resource "aws_eks_node_group" "backend-node-grp" {
  ...
  # ✅ KEEP TAINT: Only database pods should run here
  taint {
    key    = "node-role"
    value  = "osdu-backend"
    effect = "NO_SCHEDULE"
...
#########################################################################
# NODE GROUP 3: FRONTEND NODES (NO TAINTS - DEFAULT SCHEDULING)
# Purpose: OSDU APIs, Airflow, Redis, and any other workloads
#########################################################################
...
resource "aws_eks_node_group" "frontend-node-grp" {
  ...
  # ✅ NO TAINTS: Allow any pods to schedule here (default + OSDU)
  # This makes frontend nodes the default scheduling destination
...
EOT
  ...
  # Frontend-specific user data (NO SPECIAL TAINTS)
  frontend-node-userdata = <<-EOT
${local.base-node-userdata}
